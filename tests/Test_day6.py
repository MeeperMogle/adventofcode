...
class MyTestCase(unittest.TestCase):
    def test_something(self):
        ...
        # and so on, the 7 blocks are spread out over the memory banks. The fourth, first, and second banks get two
        # blocks each, and the third bank gets one back. The final result looks like this: 2 4 1 2.
        # Next, the second bank is chosen because it contains the most blocks (four). Because there are four memory
        # banks, each gets one block. The result is: 3 1 2 3.
        # Now, there is a tie between the first and fourth memory banks, both of which have three blocks. The first
        # bank wins the tie, and its three blocks are distributed evenly over the other three banks, leaving it with
        # none: 0 2 3 4.
        ...
        input_list = list(map(lambda c: int(c), '0\t2\t7\t0'.split('\t')))
        self.assertEqual(steps_until_earlier_state_reached(input_list), 5)
